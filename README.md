...
### Known Bugs
- At times, the payment_intent.succeeded can fail which results in the confirmation email not being sent out. Other times it works and the email is sent and received instantly. In Development, the webhook succeeds 100% of the time but in live it is a little more temperamental. Whilst testing, order emails confirmations were received an hour after the order was placed and in one case, a day after the order was placed. At time of submission, the payment_intent.succeeded webhook was successful when placing an order in Production.
